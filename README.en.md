...
The interactive login page, environment check, list scraping, and smoke test use foreground temporary tabs because some Chrome versions cannot obtain BOSS search responses from background targets; an automated run may therefore activate a BOSS tab briefly. Detail scraping still uses background temporary tabs with the visibility override and focus emulation, so individual detail pages do not repeatedly steal focus. The dedicated Chrome always runs with a visible UI and can be inspected through VNC when needed.
