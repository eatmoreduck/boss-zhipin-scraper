...
`--setup-chrome` 的交互式登录页、环境检查、列表抓取和 smoke test 会使用前台临时标签页，以兼容部分 Chrome 版本中后台 Target 无法取得 BOSS 搜索响应的问题；自动任务运行时可能短暂切换 BOSS 标签页。详情抓取仍使用后台临时标签页，并保留 visibility override 与焦点仿真，避免逐条详情反复抢占当前窗口。专用 Chrome 始终以有界面模式运行，必要时可以通过 VNC 检查。
